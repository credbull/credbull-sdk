...
test.describe('Test ERC20 Approve with Safe using Thirdweb', () => {
  ...
  const erc20: ERC20 = new ERC20(new CredbullClient(chainConfig));

  test('Test ERC20 Approve - Single signer', async () => {
    const safeClient: CredbullSafeClient = safeClientSingleSigner(deployerPrivateKey);

    const usdcApproveTxn: Hex = await encode(erc20.approveTxn(chainConfig.usdc, spenderAddress, depositAmount));

    const safeTxnResult = await safeClient.call(chainConfig.usdc, usdcApproveTxn);
